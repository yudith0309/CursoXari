...
namespace XafDemo.Module.E2E.Tests;

public class XafDemoTests : IDisposable {
    const string BlazorAppName = "XafDemoBlazor";
    const string WinAppName = "XafDemoWin";
    const string AppDBName = "XafDemo";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

	public XafDemoTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\XafDemo.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\XafDemo.Win\bin\EasyTest\net8.0-windows\XafDemo.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "XafDemoEasyTest", server: @"(localdb)\mssqllocaldb"));
	}
    public void Dispose() {
